...
public:
    ...
    auto checkForMatch(ClientId client_id, OrderId client_order_id,
                       TickerId ticker_id, Side side, Price price,
                       Qty qty, OrderId market_order_id) noexcept -> Qty;
    ...
    auto addOrdersAtPrice(MEOrdersAtPrice *new_orders_at_price) noexcept -> void {
        ...
        if(UNLIKELY(!best_orders_by_price)){
            (new_orders_at_price->side_
                ==Side::BUY?bids_by_price_:asks_by_price_)=new_orders_at_price;
            new_orders_at_price->prev_entry_=
            new_orders_at_price->next_entry_=new_orders_at_price;
        }
        else{
            ...
            if(add_after){
                ...
                best_orders_by_price->prev_entry_=new_orders_at_price;
                new_orders_at_price->next_entry_=best_orders_by_price;
    ...
    auto priceToIndex(Price price) const noexcept -> size_t {
