...
namespace Exchange
{
                          ...
                          Qty qty) noexcept -> void
    {
        ...
        const auto leaves_qty = checkForMatch(
            client_id, client_order_id, ticker_id, side, price, qty,
            new_market_order_id);
        if (LIKELY(leaves_qty != 0))
        {
            const auto priority = getNextPriority(price);
            auto *order = order_pool_.allocate(
                ticker_id, client_id, client_order_id, new_market_order_id,
                side, price, leaves_qty, priority, nullptr, nullptr);
            addOrder(order);

            market_update_ = {MarketUpdateType::ADD, new_market_order_id,
    ...
    auto MEOrderBook::match(TickerId ticker_id, ClientId client_id, Side side,
                            ...
                            MEOrder *order, Qty *leaves_qty) noexcept -> void
    {
        ...
        *leaves_qty -= fill_qty;
        ...
        client_response_ = {ClientResponseType::FILLED, client_id, ticker_id,
                            client_order_id, new_market_order_id, side,
                            order->price_, fill_qty, *leaves_qty};
        matching_engine_->sendClientResponse(&client_response_);

        client_response_ = {ClientResponseType::FILLED, order->client_id_, ticker_id_,
                            order->client_order_id_, order->market_order_id_,
                            order->side_, order->price_, fill_qty, order->qty_};
        matching_engine_->sendClientResponse(&client_response_);

        market_update_ = {MarketUpdateType::TRADE, OrderId_INVALID, ticker_id,
                          side, order->price_, fill_qty, Priority_INVALID};
        matching_engine_->sendMarketUpdate(&market_update_);
    ...
    auto MEOrderBook::checkForMatch(ClientId client_id, OrderId client_order_id,
                                    TickerId ticker_id, Side side, Price price,
                                    Qty qty, OrderId new_market_order_id) noexcept -> Qty
    {
        auto leaves_qty = qty;
        if (side == Side::BUY) {
            while (leaves_qty && asks_by_price_) {
                auto *ask = asks_by_price_->first_me_order_;
                if (price < ask->price_)
                    break;
                match(ticker_id, client_id, side, client_order_id,
                      new_market_order_id, ask, &leaves_qty);
            }
        } else if (side == Side::SELL) {
            while (leaves_qty && bids_by_price_) {
                auto *bid = bids_by_price_->first_me_order_;
                if (price > bid->price_)
                    break;
                match(ticker_id, client_id, side, client_order_id,
                      new_market_order_id, bid, &leaves_qty);
            }
        }
        return leaves_qty;
    }
