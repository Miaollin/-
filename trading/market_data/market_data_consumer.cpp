...
namespace Trading
{
    ...
    auto MarketDataConsumer::start() -> void
    {
        ...
        ASSERT(Common::createAndStartThread(-1, "Trading/MarketDataConsumer", [this]()
                                            { run(); }) != nullptr,
               "Fail to start MarketData thread");
    }
    ...
    auto MarketDataConsumer::run() -> void
    {
        ...
        if (socket->next_rcv_valid_index_ >= sizeof(Exchange::MDPMarketUpdate))
        {
            ...
            for (; i + sizeof(Exchange::MDPMarketUpdate) <= socket->next_rcv_valid_index_; i += sizeof(Exchange::MDPMarketUpdate))
            {
                ...
                else
                {
                    ...
                    auto next_write = incoming_md_updates_->getNextToWrite();
    ...
    auto MarketDataConsumer::checkSnapshotSync() -> void
    {
        ...
        for (const auto &[seq, msg] : snapshot_queued_msgs_)
        {
            if (seq != expected_seq)
            {
                logger_.log("%:% %() % Detected gap in snapshot stream expected:% found:% %.\n", __FILE__, __LINE__, __FUNCTION__,
                            Common::getCurrentTimeStr(&time_str_), expected_seq, seq, msg.toString());
                break;
            }
            ++expected_seq;
        }
        std::vector<Exchange::MEMarketUpdate> final_events;
        final_events.reserve(snapshot_queued_msgs_.size()-2);
        for(const auto& [seq,msg]:snapshot_queued_msgs_){
            if(msg.type_!=Exchange::MarketUpdateType::SNAPSHOT_START&&msg.type_!=Exchange::MarketUpdateType::SNAPSHOT_END){
                final_events.push_back(msg);
            }
        }
        ...
        {
            logger_.log("%:% %() % Returning because have gaps in queued incremental.\n",
                        __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_));
            snapshot_queued_msgs_.clear();
            return;
        }
        ...
        logger_.log("%:% %() % Recovered % snapshot and % incremental orders.\n", __FILE__, __LINE__, __FUNCTION__,
                    Common::getCurrentTimeStr(&time_str_), snapshot_queued_msgs_.size() - 2, num_incremental);
