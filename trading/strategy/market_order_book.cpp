...
namespace Trading
{
    MarketOrderBook::MarketOrderBook(TickerId ticker_id, Logger *logger) : ticker_id_(ticker_id),
    ...
    auto MarketOrderBook::updateBBO(bool update_bid, bool update_ask) noexcept -> void
    {
        if (update_bid)
        {
            if (bids_by_price_)
            {
                ...
                bbo_.bid_qty_ = bids_by_price_->total_qty_;
            }
            else
            {
        ...
        if (update_ask)
        {
            if (asks_by_price_)
            {
                ...
                bbo_.ask_qty_ = asks_by_price_->total_qty_;
            }
            else
            {
