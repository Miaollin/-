...
MarketDataPublisher::MarketDataPublisher(
    ...
    ASSERT(incremental_socket_.init(incremental_ip, iface, incremental_port,
           ...
           "Unable to create incremental multicast socket. error:" +
...
auto MarketDataPublisher::start() -> void {
    if (run_.exchange(true))
        return;
    thread_ = Common::createAndStartThread(
        -1, "Exchange/MarketDataPublisher", [this]() { run(); });
    ASSERT(thread_ != nullptr, "Failed to start MarketDataPublisher thread");
