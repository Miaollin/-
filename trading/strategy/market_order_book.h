...
namespace Trading
{
    ...
    private:
        ...
        MarketOrdersAtPrice *asks_by_price_;
    ...
    private:
        ...
        auto addOrdersAtPrice(MarketOrdersAtPrice *new_orders_at_price) noexcept -> void
        {
        ...
        auto removeOrder(MarketOrder *order) noexcept -> void
        {
            ...
            orders_at_price->total_qty_ -= order->qty_;
            if (order->prev_order_ == order)
            {
                ASSERT(
                    orders_at_price->total_qty_ == 0,
                    "Expected empty price level");
        ...
        auto addOrder(MarketOrder *order) noexcept -> void
        {
            ...
            if (!orders_at_price)
            {
                ...
                new_orders_at_price->total_qty_ = order->qty_;
                addOrdersAtPrice(new_orders_at_price);
            }
            else
            {
                auto first_order = orders_at_price->first_mkt_order_;
